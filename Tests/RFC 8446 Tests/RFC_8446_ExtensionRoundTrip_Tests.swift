...
struct RFC_8446_ExtensionRoundTrip_Tests {
    ...
    func `Extension.Data envelope round-trips`() throws {
        let ext = RFC_8446.Extension.Data(type: .cookie, data: [Byte(1), Byte(2), Byte(3)])
        #expect(try RFC_8446.Extension.Data(binary: ext.bytes) == ext)
    }

    @Test
    func `SupportedGroups round-trips`() throws {
        let value = RFC_8446.Extension.SupportedGroups(namedGroupList: [.x25519, .secp256r1, .x448])
    ...
    func `SignatureAlgorithms round-trips`() throws {
        let value = RFC_8446.Extension.SignatureAlgorithms(
    ...
    func `SignatureAlgorithmsCert round-trips`() throws {
        let value = RFC_8446.Extension.SignatureAlgorithmsCert(
    ...
    func `Cookie round-trips`() throws {
        let value = RFC_8446.Extension.Cookie(cookie: [Byte(0xC0), Byte(0x0C), Byte(0x1E)])
    ...
    func `Padding round-trips`() {
        let value = RFC_8446.Extension.Padding(length: 8)
    ...
    func `PskKeyExchangeModes round-trips`() throws {
        let value = RFC_8446.Extension.PskKeyExchangeModes(keModes: [.pskDheKe, .pskKe])
    ...
    func `KeyShare ClientHello round-trips`() throws {
        let value = RFC_8446.Extension.KeyShare.ClientHello(clientShares: [
            RFC_8446.Extension.KeyShare.Entry(group: .x25519, keyExchange: Array(repeating: Byte(0xAB), count: 32)),
            RFC_8446.Extension.KeyShare.Entry(group: .secp256r1, keyExchange: Array(repeating: Byte(0xCD), count: 65)),
        ])
    ...
    func `KeyShare ServerHello round-trips`() throws {
        let value = RFC_8446.Extension.KeyShare.ServerHello(
            serverShare: RFC_8446.Extension.KeyShare.Entry(group: .x25519, keyExchange: Array(repeating: Byte(0x11), count: 32))
        )
    ...
    func `SupportedVersions ClientHello round-trips`() throws {
        let value = RFC_8446.Extension.SupportedVersions.ClientHello(versions: [.tls1_3, .tls1_2])
    ...
    func `PreSharedKey OfferedPsks round-trips`() throws {
        let value = RFC_8446.Extension.PreSharedKey.OfferedPsks(
            identities: [
                RFC_8446.Extension.PreSharedKey.Identity(identity: [Byte(0x01), Byte(0x02)], obfuscatedTicketAge: 12345),
                RFC_8446.Extension.PreSharedKey.Identity(identity: [Byte(0x03)], obfuscatedTicketAge: 0),
            ],
            binders: [
    ...
    func `CertificateAuthorities round-trips`() throws {
        let value = RFC_8446.Extension.CertificateAuthorities(authorities: [
            [Byte(0x30), Byte(0x0E)],
    ...
    func `OidFilters round-trips`() throws {
        let value = RFC_8446.Extension.OidFilters(filters: [
            RFC_8446.Extension.OidFilters.Filter(
